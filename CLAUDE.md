...
- AI enrichment service: **standalone FastAPI (Python 3.11)** — NOT the same process as the API. Deployed to Render (`aesis-ai-engine` web). One pipeline:
  ...
  - Chatbot uses **Groq** (`openai/gpt-oss-120b`, OpenAI-compatible; the id is env-set via `GROQ_MODEL` and Groq retires ids — `GET /health` on the engine reports whether the configured one still exists). **There is no Gemini/OpenAI anywhere** — do not assume an external LLM scores anything.
