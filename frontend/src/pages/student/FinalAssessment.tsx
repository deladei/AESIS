  ...
  if (!placement || isError || !data) {
    return (
      <div className="mx-auto max-w-3xl p-6">
        <h1 className="mb-4 text-xl font-bold text-[#0b1c30]">Final Assessment</h1>
        <div className="flex items-start gap-3 rounded-xl bg-[#f3f3f7] p-8 text-[#444653]">
          <Lock className="mt-0.5 h-5 w-5 shrink-0 text-[#15157d]" />
          <div>
            <p className="font-semibold text-[#0b1c30]">Not available yet</p>
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-6">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-[#0b1c30]">Final Assessment</h1>
        ...
        <span className="inline-flex items-center gap-1.5 rounded-full bg-[#e9f9ef] px-3 py-1 text-xs font-semibold text-[#1b7a45]">
      ...
      <section className="rounded-xl bg-white p-8 shadow-sm">
        <div className="flex items-center gap-6">
          <div className="flex h-14 w-14 items-center justify-center rounded-full bg-[#eef1ff] text-[#15157d]">
            <Award className="h-7 w-7" />
          </div>
          <div>
            <p className="text-sm font-medium text-[#757684]">Final grade</p>
            <p className="text-3xl font-extrabold text-[#0b1c30]">{data.grade ?? '—'}</p>
          </div>
        </div>
        {(data.signedOffBy || data.signedOffAt) && (
          <p className="mt-4 border-t border-[#eef1ff] pt-3 text-xs text-[#757684]">
      ...
      {data.evaluation && (data.evaluation.criteria.length > 0 || data.evaluation.recommendation) && (
        <section className="rounded-xl bg-white p-8 shadow-sm">
          <h2 className="mb-4 text-sm font-bold text-[#0b1c30]">Supervisor evaluation</h2>
          {data.evaluation.recommendation && (
            <p className="mb-4 inline-flex rounded-full bg-[#e1e0ff] px-3 py-1 text-xs font-semibold text-[#15157d]">
          ...
          <ul className="space-y-3">
            {data.evaluation.criteria.map((c, i) => (
              <li key={i} className="flex items-start justify-between gap-4 border-b border-[#f3f3f7] pb-3 last:border-0">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-[#0b1c30]">{c.criterion}</p>
                  {c.comment && <p className="text-xs text-[#757684]">{c.comment}</p>}
                </div>
                <div className="flex shrink-0 gap-0.5 text-[#15157d]">
                  {[1, 2, 3, 4, 5].map((n) => (
      ...
      {data.narrative && (
        <section className="rounded-xl bg-white p-8 shadow-sm">
          <h2 className="mb-2 text-sm font-bold text-[#0b1c30]">Narrative</h2>
          <p className="whitespace-pre-wrap text-sm text-[#444653]">{data.narrative}</p>
        </section>
      )}
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-xl bg-white p-6 shadow-sm">
          <p className="mb-2 text-sm font-bold text-[#0b1c30]">Final report</p>
          {data.finalReport ? (
            <a href={data.finalReport.fileUrl} target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-2 text-sm font-medium text-[#15157d] hover:underline">
              <FileText className="h-4 w-4" /> {data.finalReport.fileName}
            </a>
          ) : (
            <p className="text-sm text-[#757684]">Not uploaded.</p>
          )}
        </div>
        <div className="rounded-xl bg-white p-6 shadow-sm">
          <p className="mb-2 text-sm font-bold text-[#0b1c30]">Company attestation</p>
          {data.companyAttestation?.attestedAt ? (
            ...
            <p className="text-sm text-[#757684]">No attestation on file.</p>
          )}
        </div>
      </section>
    </div>
