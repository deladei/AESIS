...
export default function SupervisorUploadPanel() {
  ...
  // Keying one supervisor in by hand. Deliberately the SAME mutation the
  // spreadsheet path uses — one row instead of many — so validation, the
  // created/updated/skipped summary and the error handling stay in one place
  // rather than being written twice and drifting.
  const [manual, setManual] = useState(false);
  const [one, setOne] = useState({ firstName: '', lastName: '', email: '', region: '' });
  const oneReady = one.firstName.trim() !== '' && one.lastName.trim() !== ''
    && EMAIL_RE.test(one.email.trim()) && one.region !== '';
  ...
  const addOne = () => {
    ...
    bulk.mutate(
      [{
        ...
        email: one.email.trim().toLowerCase(), region: one.region,
      }],
      {
        onSuccess: (res) => {
          setResult(res);
          setOne({ firstName: '', lastName: '', email: '', region: '' });
        },
        onError: () => setError('Could not add that supervisor — please try again.'),
      },
    );
  };
  ...
  return (
    <section className="rounded-card border border-line bg-surface p-6">
      ...
      {/* Manual entry — for the one supervisor who arrives after the
      ...
      <div className="mt-4 border-t border-line pt-4">
        {!manual ? (
          ...
          >
            <Users className="h-4 w-4" /> Or add one supervisor by hand
          </button>
        ) : (
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-2">
              <p className="text-sm font-semibold text-ink">Add a supervisor</p>
              <button
                type="button"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <input
                ...
                className="rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink outline-none focus:ring-1 focus:ring-brand"
              />
              {/* A picker, not free text: the region is an enum, and typing it
                  is how "Greater-Accra" becomes an unrecognised row. */}
              <select
                value={one.region}
                onChange={(e) => setOne((o) => ({ ...o, region: e.target.value }))}
                className="rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink outline-none focus:ring-1 focus:ring-brand"
              >
                <option value="">Region covered…</option>
                {REGION_VALUES.map((v) => (
                  <option key={v} value={v}>{REGION_LABELS[v]}</option>
                ))}
              </select>
            </div>
            ...
            <button
              type="button"
              onClick={addOne}
              disabled={!oneReady || bulk.isPending}
