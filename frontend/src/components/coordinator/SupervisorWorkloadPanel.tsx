    ...
    <div className="rounded-xl border border-[#c4c5d5]/60 bg-white p-6">
      <div className="mb-4 flex items-start justify-between">
        <div className="flex items-center gap-2">
          <Users2 className="h-5 w-5 text-[#15157d]" />
          <h3 className="text-lg font-semibold text-[#15157d]">Supervisor Workload</h3>
        </div>
      ...
      {isLoading ? (
        ...
        <p className="py-6 text-center text-sm text-[#757684]">No academic supervisors yet.</p>
      ) : (
        <>
          {data && (
          ...
          <ul className="space-y-3">
            {rows.map((r) => (
              <li key={r.supervisor.id} className="flex items-center gap-3">
                <span className="w-40 shrink-0 truncate text-sm font-medium text-[#0b1c30]" title={r.supervisor.name}>
                  {r.supervisor.name}
                </span>
                <div className="h-2 flex-1 overflow-hidden rounded-full bg-[#eef0f5]">
                  <div
                    className={`h-full rounded-full ${r.overloaded ? 'bg-amber-500' : 'bg-[#15157d]'}`}
                    style={{ width: `${max > 0 ? Math.round((r.internCount / max) * 100) : 0}%` }}
                  />
                </div>
                <span className={`w-8 shrink-0 text-right text-sm font-semibold ${r.overloaded ? 'text-amber-600' : 'text-[#0b1c30]'}`}>
                  {r.internCount}
                </span>
              </li>
            ))}
          ...
          {data && data.unassigned > 0 && (
            <div className="mt-4 flex items-center gap-2 border-t border-[#eef1ff] pt-3 text-sm text-[#b3261e]">
