...
const STATUS_META: Record<string, { label: string; cls: string }> = {
  submitted:    { label: 'Submitted',    cls: 'bg-info-soft text-brand-ink' },
  acknowledged: { label: 'Acknowledged', cls: 'bg-ok-soft text-ok' },
  returned:     { label: 'Returned',     cls: 'bg-danger-soft text-danger' },
  draft:        { label: 'Draft',        cls: 'bg-warn-soft text-warn' },
  not_started:  { label: 'Not started',  cls: 'bg-surface-sunken text-ink-secondary' },
};
...
function SortHeader({
  ...
  return (
    <th className={`px-4 py-3 ${className}`}>
      <button
        onClick={() => onSort(col)}
        className={`inline-flex items-center gap-1 font-semibold tracking-wide transition-colors hover:text-brand-ink ${active ? 'text-brand-ink' : 'text-ink-muted'}`}
      >
        {label}
...
function InternRow({ s, selected, onToggle }: { s: CoordinatorStudent; selected: boolean; onToggle: (id: string) => void }) {
  ...
  const lastEntry = s.lastSubmittedAt
    ...
    : '—';
  return (
    <tr className={`transition-colors hover:bg-surface-sunken ${selected ? 'bg-surface-sunken' : ''}`}>
        ...
        <Link to={`/coordinator/interns/${s.placementId}`} className="flex items-center gap-3">
          ...
          <div>
            <p className="flex items-center gap-1 text-sm font-bold leading-tight text-ink">
              {name}
              {s.flagged && <Flag className="h-3 w-3 fill-warn text-warn" />}
            </p>
            <p className="font-mono text-xs text-ink-muted">#{s.placementId.slice(0, 6).toUpperCase()}</p>
          </div>
        </Link>
      </td>
      <td className="px-4 py-3 text-sm text-ink">{s.department ?? '—'}</td>
      <td className="px-4 py-3 text-sm text-ink">
        {s.supervisor?.name?.trim() ? s.supervisor.name : <span className="text-ink-muted">Unassigned</span>}
      </td>
      <td className="px-4 py-3">
      ...
      <td className="px-4 py-3">
        ...
        >
          <div className="mb-1 flex justify-between text-[10px]">
            <span className="font-bold text-brand-ink">Week {s.lastWeek ?? 0} of {s.programmeWeeks}</span>
            <span className="text-ink-muted">{s.progressPct != null ? `${s.progressPct}%` : '—'}</span>
          </div>
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-surface-sunken">
            <div className="h-full bg-brand" style={{ width: `${Math.min(100, Math.max(0, s.progressPct ?? 0))}%` }} />
          </div>
        </Link>
      </td>
...
export default function InternStatusTable({ pageSize = 20, viewAllHref, scopeYearId, initialFilters }: Props) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-line bg-surface shadow-sm">
      <div className="flex items-center justify-between border-b border-line bg-surface-sunken px-6 py-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="rounded-lg border border-line px-2 py-1.5 text-ink-secondary transition-colors hover:text-brand-ink"
          >
          ...
          <button
            ...
            className={`relative rounded-lg border px-2 py-1.5 transition-colors ${showFilters || activeFilters ? 'border-brand bg-brand-soft text-brand-ink' : 'border-line text-ink-secondary hover:text-brand-ink'}`}
          >
            <Filter className="h-4 w-4" />
            {activeFilters > 0 && (
              <span className="absolute -right-1.5 -top-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-brand text-[9px] font-bold text-white">{activeFilters}</span>
            )}
          </button>
        </div>
      </div>
      ...
      {selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-2 border-b border-line bg-surface-sunken px-6 py-2.5 text-sm">
          <span className="font-semibold text-brand-ink">{selected.size} selected</span>
          <button onClick={doBulkRemind} disabled={bulkRemind.isPending} className="inline-flex items-center gap-1.5 rounded-lg border border-line bg-surface px-3 py-1.5 text-xs font-semibold text-ink-secondary transition-colors hover:text-brand-ink disabled:opacity-50">
            <BellRing className="h-3.5 w-3.5" /> Send reminder
          </button>
          <button onClick={() => setBulkAssignOpen(true)} className="inline-flex items-center gap-1.5 rounded-lg border border-line bg-surface px-3 py-1.5 text-xs font-semibold text-ink-secondary transition-colors hover:text-brand-ink">
            <UserCheck className="h-3.5 w-3.5" /> Assign supervisor
          </button>
          <button onClick={doExport} className="inline-flex items-center gap-1.5 rounded-lg border border-line bg-surface px-3 py-1.5 text-xs font-semibold text-ink-secondary transition-colors hover:text-brand-ink">
            <Download className="h-3.5 w-3.5" /> Export
          </button>
          <button onClick={clearSel} className="ml-auto text-xs font-medium text-ink-muted transition-colors hover:text-danger">Clear</button>
        </div>
      )}
      ...
      {showFilters && (
        <div className="flex flex-wrap items-center gap-2 border-b border-line bg-surface px-6 py-3">
          ...
          {activeFilters > 0 && (
            <button onClick={clearFilters} className="inline-flex items-center gap-1 text-sm font-medium text-ink-muted transition-colors hover:text-danger">
        ...
        <table className="w-full text-left">
          <thead className="bg-surface-sunken text-xs">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold tracking-wide text-ink-muted">Attention</th>
              ...
              <th className="px-6 py-3 text-right text-xs font-semibold tracking-wide text-ink-muted">Action</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-line">
            {isLoading ? (
              <tr><td colSpan={9} className="px-6 py-10 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin text-brand-ink" /></td></tr>
            ) : students.length === 0 ? (
      ...
      {viewAllHref
        ? meta && meta.total > students.length && (
            <div className="flex justify-center border-t border-line bg-surface-sunken py-4">
              <Link to={viewAllHref} className="text-sm font-semibold text-brand-ink hover:underline">
                View all {meta.total.toLocaleString()} interns
              </Link>
            </div>
          )
        : meta && meta.total > 0 && (
            <div className="flex items-center justify-between border-t border-line bg-surface-sunken px-6 py-3 text-sm">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={!meta.hasPrevPage}
                  className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-line text-ink-secondary transition-colors hover:bg-brand-soft disabled:cursor-not-allowed disabled:opacity-40" aria-label="Previous page">
                  <ChevronLeft className="h-4 w-4" />
                </button>
                <span className="px-2 font-medium text-ink">Page {meta.page} of {meta.totalPages || 1}</span>
                <button onClick={() => setPage((p) => p + 1)} disabled={!meta.hasNextPage}
                  className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-line text-ink-secondary transition-colors hover:bg-brand-soft disabled:cursor-not-allowed disabled:opacity-40" aria-label="Next page">
      ...
      {bulkAssignOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={() => setBulkAssignOpen(false)}>
          <div className="w-full max-w-md rounded-xl bg-surface p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <select value={bulkSupId} onChange={(e) => setBulkSupId(e.target.value)} className="w-full rounded-lg border border-line px-3 py-2 text-sm focus:border-brand focus:outline-none">
              <option value="">Select a supervisor…</option>
              {supervisors.map((s) => <option key={s.id} value={s.id}>{s.firstName} {s.lastName}</option>)}
            </select>
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={() => setBulkAssignOpen(false)} className="rounded-lg border border-line px-4 py-2 text-sm font-medium text-ink-secondary hover:bg-surface-sunken">Cancel</button>
              <button onClick={doBulkAssign} disabled={!bulkSupId || bulkAssign.isPending} className="inline-flex items-center gap-2 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white hover:opacity-90 disabled:opacity-50">
